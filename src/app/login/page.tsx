...
export default function LoginPage() {
  ...
  const handleGoogleSignIn = () => {
    // Use a custom callback URL that will handle the user flow
    signIn("google", {
      callbackUrl: "/api/auth/callback/check-user",
      ...
      <Card className="w-full max-w-md bg-[hsl(var(--card))] text-[hsl(var(--foreground))]">
        <CardHeader className="text-center">
          <div className="flex justify-center mb-4">
            <PiggyBank className="h-12 w-12 text-[hsl(var(--primary))]" />
          </div>
          <CardTitle className="text-2xl">Welcome to Eno</CardTitle>
          <CardDescription>
            Sign in with Google to start tracking your savings journey
          </CardDescription>
        </CardHeader>
        <CardContent>
          <Button
